...
import com.cpm.Marico_so.getterSetter.MenuGetterSetter;
...
public class EntryMenuActivity extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        if(getIntent().getSerializableExtra(CommonString.KEY_STATUS) != null && getIntent().getStringExtra(CommonString.KEY_IMAGE) != null){
            ...
            shareIntent.putExtra(Intent.EXTRA_STREAM, FileProvider.getUriForFile(EntryMenuActivity.this,"com.cpm.Marico_so.fileprovider",photoFile));
    ...
    protected void onResume() {
        ...
        if(journeyPlan.getAuditEnable().equalsIgnoreCase("Y")) {
            menu_list = database.getMenuData(journeyPlan.getStoreTypeId(), journeyPlan.getStoreCategoryId(),true);
        }else{
            menu_list = database.getMenuData(journeyPlan.getStoreTypeId(), journeyPlan.getStoreCategoryId(),false);
        }
        ...
        public void onBindViewHolder(final MyViewHolder viewHolder, final int position) {
            ...
            switch (current.getMenuId()) {
                ...
                case 10:
                    ...
                    if(database.getTesterStockData(journeyPlan.getStoreTypeId(), String.valueOf(journeyPlan.getStateId()), journeyPlan.getStoreCategoryId()).size() >0){
                ...
                case 11:
                    if(database.getAuditQuestionCategoryData(journeyPlan).size() >0){
                        if (database.isAuditDataFilled(journeyPlan.getStoreId().toString())) {
                            icon_path = current.getTickIcon();
                        } else {
                            icon_path = current.getNormalIcon();
                        }
                ...
                public void onClick(View v) {
                    ...
                    switch (menu_id) {
                        ...
                        case 8:
                            ...
                            if (!database.isgROOMINGFilled(journeyPlan)) {
                                startActivity(new Intent(context, GroomingImageActivity.class).putExtra(CommonString.TAG_OBJECT, journeyPlan).putExtra(CommonString.KEY_MENU_ID, current));
                                overridePendingTransition(R.anim.activity_in, R.anim.activity_out);
                            } else {
                                Snackbar.make(recyclerView, "Data cannot be changed", Snackbar.LENGTH_SHORT).show();
                            }
                        ...
                        case 11:
                            ...
                            if(database.getAuditQuestionCategoryData(journeyPlan).size() > 0){
                                if (!database.isAuditDataFilled(journeyPlan.getStoreId().toString())) {
                                    startActivity(new Intent(context, QuizActivity.class).putExtra(CommonString.TAG_OBJECT, journeyPlan).putExtra(CommonString.KEY_MENU_ID, current));
                                    overridePendingTransition(R.anim.activity_in, R.anim.activity_out);
                                }else{
                                    Snackbar.make(recyclerView, "Audit data already saved", Snackbar.LENGTH_SHORT).show();
                                }
                            }
                            break;
                    }
    ...
    void declaration() {
        ...
        database = new MaricoSODatabase(context);
        database.open();
    }
